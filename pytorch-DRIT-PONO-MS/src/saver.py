...
class Saver():
  ...
  # save result images
  def write_img(self, total_it, model):
    ...
    assembled_images = model.assemble_outputs()
    img_filename = '%s/gen_%07d.jpg' % (self.image_dir, total_it)
    torchvision.utils.save_image(assembled_images / 2 + 0.5, img_filename, nrow=1)

  # save model
  def write_model(self, ep, total_it, model):
    model.save('{}/last.pth'.format(self.model_dir), ep, total_it)
    print('--- save the model @ iter %d ---' % (total_it))
    model.save('{}/iter{:07d}.pth'.format(self.model_dir, total_it), ep, total_it)

  def write_model_old(self, ep, total_it, model):
    if ep == -1:
      model.save('%s/last.pth' % self.model_dir, ep, total_it)
    elif (ep + 1) % self.model_save_freq == 0:
