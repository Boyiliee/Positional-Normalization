...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'restnetPONOMS_9block':
        net = ResnetPONOMSGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnetPONODMS_9blocks':
        net = ResnetPONODMSGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
...
class ResnetPONODMSGenerator(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetPONODMSGenerator, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        reflection_block = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            down_model += [Conv2dBlock(ngf * mult, ngf * mult * 2, 'conv', use_bias, pono=self.pono)]


        mult = 2 ** n_downsampling
        res_block = []
        for i in range(n_blocks):       # add ResNet blocks
            res_block += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        self.stat_convs = nn.ModuleList()

        up_model = []
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            up_model += [Conv2dBlock(ngf * mult, int(ngf * mult / 2), 'deconv', use_bias, ms=self.ms)]
            net = nn.Sequential(Conv2dBlock(2, int(ngf * mult / 2), 'deconv', use_bias),
                                Conv2dBlock(int(ngf * mult / 2), (ngf * mult), 'stat_convs', use_bias, norm=False))
            self.stat_convs.append(net)

        out_block = [nn.ReflectionPad2d(3)]
        out_block += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        out_block += [nn.Tanh()]
    ...
    def forward(self, input):
        """Standard forward"""
        x = self.reflection_block(input)

        stats = []
        for block in self.down_model:
            x, mean, std = block(x)
...
class ResnetPONOMSGenerator(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetPONOMSGenerator, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        self.upsamle = nn.Upsample(scale_factor=2)

        up_model = []
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            up_model += [Conv2dBlock(ngf * mult, int(ngf * mult / 2), 'deconv', use_bias)]
